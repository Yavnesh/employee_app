...
class _CustomCalendarDatePickerState extends State<CustomCalendarDatePicker> {
  ...
  void didUpdateWidget(CustomCalendarDatePicker oldWidget) {
    ...
    if (!isSameDay(widget.initialDate, oldWidget.initialDate)) {
      _currentDisplayedMonthDate = DateTime(widget.initialDate.year, widget.initialDate.month);
      _selectedDate = widget.initialDate;
    }
  }
  ...
  void didChangeDependencies() {
    ...
    if (!_announcedInitialDate) {
      ...
      final bool isToday = isSameDay(widget.currentDate, _selectedDate);
...
class _MonthPickerState extends State<_MonthPicker> {
  ...
  void _handleMonthPageChanged(int monthPage) {
    setState(() {
      ...
      if (!isSameMonth(_currentMonth, monthDate)) {
        _currentMonth = DateTime(monthDate.year, monthDate.month);
        widget.onDisplayedMonthChanged(_currentMonth);
        if (_focusedDay != null && !isSameMonth(_focusedDay, _currentMonth)) {
  ...
  DateTime? _focusableDayForMonth(DateTime month, int preferredDay) {
    final int daysInMonth = getDaysInMonth(month.year, month.month);
  ...
  void _showMonth(DateTime month, { bool jump = false}) {
    final int monthPage = monthDelta(widget.firstDate, month);
    if (jump) {
  ...
  void _handleGridFocusChange(bool focused) {
    setState(() {
      if (focused && _focusedDay == null) {
        if (isSameMonth(widget.selectedDate, _currentMonth)) {
          _focusedDay = widget.selectedDate;
        } else if (isSameMonth(widget.currentDate, _currentMonth)) {
  ...
  void _handleDirectionFocus(DirectionalFocusIntent intent) {
    ...
    setState(() {
      ...
      if (nextDate != null) {
        ...
        if (!isSameMonth(_focusedDay, _currentMonth)) {
  ...
  DateTime? _nextDateInDirection(DateTime date, TraversalDirection direction) {
    ...
    DateTime nextDate = addDaysToDate(date, _dayDirectionOffset(direction, textDirection));
    while (!nextDate.isBefore(widget.firstDate) && !nextDate.isAfter(widget.lastDate)) {
      if (_isSelectable(nextDate)) {
        return nextDate;
      }
      nextDate = addDaysToDate(nextDate, _dayDirectionOffset(direction, textDirection));
    }
    return null;
  }
...
class _FocusedDate extends InheritedWidget {
  ...
  bool updateShouldNotify(_FocusedDate oldWidget) {
    return !isSameDay(date, oldWidget.date);
  }
...
class _DayPickerState extends State<_DayPicker> {
  ...
  void initState() {
    super.initState();
    final int daysInMonth = getDaysInMonth(widget.displayedMonth.year, widget.displayedMonth.month);
  ...
  void didChangeDependencies() {
    ...
    if (focusedDate != null && isSameMonth(widget.displayedMonth, focusedDate)) {
  ...
  Widget build(BuildContext context) {
    ...
    while (day < daysInMonth) {
      ...
      if (day < 1) {
        ...
        final bool isSelectedDay = isSameDay(widget.selectedDate, dayToBuild);
        final bool isToday = isSameDay(widget.currentDate, dayToBuild);
...
class YearPicker extends StatefulWidget {
  ...
  YearPicker({
    ...
    this.dragStartBehavior = DragStartBehavior.start,
        ...
        currentDate = dateOnly(currentDate ?? DateTime.now()),
        initialDate = dateOnly(initialDate ?? selectedDate);
