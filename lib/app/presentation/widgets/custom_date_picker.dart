...
class _DatePickerDialogState extends State<DatePickerDialog> with RestorationMixin {
  ...
  void handleToday(DateTime date){
    setState(() {
      _selectedDate.value = date;
    });
  }
  ...
  Widget build(BuildContext context) {
        ...
        children: <Widget>[
          Row(
            children: [
              ...
              Text(localizations.formatShortDate(_selectedDate.value), style: const TextStyle(fontSize: 16.0),),
            ],
          ),
          Row(
            children: [
    ...
    final Widget headerFrom = SizedBox(
      height: 120.0,
      child: Material(
        color: Colors.white,
        child: Padding(
                ...
                children: [
                  ...
                  Expanded(
                    child: ElevatedButton(
                      ...
                      onPressed: (){
                        num daysUntilNextMonday = 7 - (_selectedDate.value.weekday - DateTime.monday);
                        DateTime nextMonday = _selectedDate.value.add(Duration(days: daysUntilNextMonday.toInt()));
                        setState(() {
                          _selectedDate.value = nextMonday;
                        });
                      },
                ...
                children: [
                  Expanded(
                    child: ElevatedButton(
                      ...
                      onPressed: (){
                        num daysUntilNextTuesday = DateTime.tuesday - _selectedDate.value.weekday;
                        if (daysUntilNextTuesday <= 0) {
                          daysUntilNextTuesday += 7;
                        }
                        DateTime nextTuesday = _selectedDate.value.add(Duration(days: daysUntilNextTuesday.toInt()));
                        setState(() {
                          _selectedDate.value = nextTuesday;
                        });
                      },
                  ...
                  Expanded(
                    child: ElevatedButton(
                      ...
                      onPressed: (){
                        DateTime selectedDate = _selectedDate.value; // Replace with your selected date
                        Duration oneWeek = const Duration(days: 7);
                        DateTime oneWeekLater = selectedDate.add(oneWeek);
                        setState(() {
                          _selectedDate.value = oneWeekLater;
                        });
                      },
    ...
    final Widget headerTo = SizedBox(
      height: 80.0,
      child: Material(
        color: Colors.white,
        child: Padding(
    ...
    //   ),
    //   titleText: localizations.formatShortDate(_selectedDate.value),
            ...
            switch (orientation) {
              case Orientation.portrait:
                return Column(
                  ...
                  children: <Widget>[
                    widget.isFromDate ? headerFrom : headerTo,
                    Expanded(child: picker),
                    if (useMaterial3) Divider(color: Colors.grey.shade300,),
                    actions,
                  ],
                );
              case Orientation.landscape:
...
//   }
//
//   void _handleNextMonday() {
//   }
//
//   void _handleAfter1Week() {
//   }
//
//   void _handleNextTuesday() {
//   }
// }
...
class _CalendarDateRangePicker extends StatefulWidget {
  ...
  _CalendarDateRangePicker({
    ...
    required this.onStartDateChanged,
    required this.onEndDateChanged,
  }) : initialStartDate = initialStartDate != null ? dateOnly(initialStartDate) : null,
        initialEndDate = initialEndDate != null ? dateOnly(initialEndDate) : null,
        firstDate = dateOnly(firstDate),
        lastDate = dateOnly(lastDate),
        currentDate = dateOnly(currentDate ?? DateTime.now()) {
    assert(
...
class _InputDateRangePicker extends StatefulWidget {
  ...
  _InputDateRangePicker({
    ...
    this.keyboardType = TextInputType.datetime,
  }) : initialStartDate = initialStartDate == null ? null : dateOnly(initialStartDate),
        initialEndDate = initialEndDate == null ? null : dateOnly(initialEndDate),
        firstDate = dateOnly(firstDate),
        lastDate = dateOnly(lastDate);
